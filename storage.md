...
```bash sudo apt install mdadm -y```

2. Crear el arreglo RAID 1
Suponiendo que los discos disponibles son /dev/sdb y /dev/sdc:
```bash sudo mdadm --create --verbose /dev/md0 --level=1 --raid-devices=2 /dev/sdb /dev/sdc```
...
```bash sudo mkdir -p /mnt/raid1```
```bash sudo mount /dev/md0 /mnt/raid1```

6. Configurar montaje automático
Editar /etc/fstab y agregar:
```bash /dev/md0   /mnt/raid1   ext4   defaults   0   0```
